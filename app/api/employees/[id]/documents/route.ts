...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    const employeeId = params.id // <-- use as string
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (existingDoc.length > 0) {
      ...
      // Insert new document
      await sql`
        INSERT INTO employee_documents (employee_id, document_type, file_name, file_path, file_size, file_url)
        VALUES (${employeeId}, ${documentType}, ${fileName}, ${publicPath}, ${file.size}, ${publicPath})
      `
    }
